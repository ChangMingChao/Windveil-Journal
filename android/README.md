...
> 现已迁至项目顶层 `android/` 目录独立演进，支持**独立模式**（数据全部存于本地 Room，无需服务端）。
...
2. 运行 `app` 到设备/模拟器；首次启动点「开始」即建立匿名个人空间。默认独立模式即可完整使用。
3. 如需连接服务端联调：`app/build.gradle.kts` 里的 `API_BASE_URL`：
   - 模拟器访问本机后端：`http://10.0.2.2:8000/`
   - 真机联调：改为局域网地址 `http://<你的IP>:8000/`（需允许明文时，在 manifest 加 `android:usesCleartextTraffic="true"`，仅限开发）
