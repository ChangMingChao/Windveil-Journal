# delta — core-00-information-architecture.md（notification-channels）
...
**S10 范围说明**：S10 复用 P6 页面（不开新路由），新增「提醒通道」开关区（推送 / 邮件两个独立开关）；交互规格见 `core-07-notification-channels-design.md`。开关切换立即生效，全部关闭时完全静默且已确认的时机顺延保留。
...
/me                     P6 我的（隐私说明、它记得我什么 [S08]、提醒通道 [S10]、导出、注销）
/me#remembered          P6「它记得我什么」区（偏好 + 可用时段，S08 常驻入口）
/me#channels            P6「提醒通道」区（推送 / 邮件开关，S10）
```

导航层级（底部固定导航，仅 3 项 + 1 个主动作）不变。S10 与 S08 同理不开新路由：通道开关是 P6 内的一块锚点区域。
...
| 静默通道 | 用户可在 P6 独立开关推送与邮件（S10）：推送关闭时投递直接走邮件；全部关闭时完全静默——outbox 中已确认时机的 pending 记录顺延保留、重开后自然恢复，应用内不再有任何催促标记 |
...
| S10 | `core-07-notification-channels-design.md` | 无新增屏（P6 开关区复用既有令牌，规格内含验收条件） |

本文档不单独配原型：信息架构与全局规则通过上述 6 份原型共同体现；S10 为 P6 内的开关区，不新增原型屏。
