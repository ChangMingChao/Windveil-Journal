# delta — core-S08-test-cases.md（preferences-availability-timing，全新文件）

> 模块：core｜上游：`../prd/3-technical-plan/2-scenario-implementation/core-S08-preferences-availability.md`（25 Steps + 摘要支线 D1–D4 / 6 EX）
> API：`../api/auth.yaml`（preferences tag）、`../api/wishes.yaml`（timing-proposals 联动）｜DB：`../database/schema.sql`（user_preferences、availability_windows、timing_proposals）

## ADDED — S08: 管理偏好与可用时段测试用例
