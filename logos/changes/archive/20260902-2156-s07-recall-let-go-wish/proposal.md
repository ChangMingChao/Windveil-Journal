...
- 影响的 smoke 测试：`test/smoke/core-smoke-test-cases.md` 与
  `scripts/smoke-core.py` — S07 迁移新增索引后，既有 `SMOKE-core-07` 的基线数量
  需要从 28 修正为 29；不新增 SMOKE ID
...
## 追加修复：SMOKE-core-07 基线漂移

S07 的 `0007_s07_recall` 迁移新增 `idx_wishes_owner_let_go_at`，因此 staging 数据库
在迁移完成后包含 17 张业务表和 29 个 `idx_` 索引。现有 smoke runner 与 smoke 用例规格
仍保留 S07 之前的 28 个索引基线，导致部署后的 Gate 3.8 误报失败。追加修复只同步
smoke 规格与断言到当前已合并的数据库设计，不改变业务代码、API、数据库迁移或部署拓扑。

本修复不需要重新部署；完成代码与测试修正后重新执行 staging smoke。`SMOKE-core-17`
因 staging 使用内部 CA 而 skip 的既有规则保持不变。
