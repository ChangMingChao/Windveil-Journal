-- delta — database/schema.sql（preferences-availability-timing）
--
-- 新增三张表：user_preferences / availability_windows / timing_proposals。
-- 全部为加表类迁移：不改动既有表的任何列；旧代码不读不写这三张表。
-- 表数量由 17 张增至 20 张（部署方案与 smoke 的表数量断言同步更新）。

## ADDED — CREATE TABLE user_preferences
...
## ADDED — CREATE TABLE availability_windows
...
## ADDED — CREATE TABLE timing_proposals
...
> 说明：新增三张表全部含 `owner_id` 并纳入 owner_guard 第一/第二道防线；越权访问统一 404（S08 EX-19.1）。

## MODIFIED — 追溯汇总：API 端点 → 表（增补段）

```text
--   auth.yaml       listPreferences / declarePreference / revokePreference
--                   / deletePreference
--                     → user_preferences, timing_proposals（撤回/删除联动失效）
--                   listAvailability / createAvailabilityWindow
--                   / updateAvailabilityWindow / deleteAvailabilityWindow
--                     → availability_windows, timing_proposals（删除联动失效）
--   wishes.yaml     createTimingProposal / listTimingProposals
--                     → timing_proposals, user_preferences, availability_windows
--                   confirmTimingProposal
--                     → timing_proposals, wishes（复用 setWishTiming 既有链路）
--                   rejectTimingProposal
--                     → timing_proposals
--   （Scheduler 低频任务：提议过期扫描 → timing_proposals；
--     偏好摘要生成 → user_preferences, LLM）
```

> 表数量断言联动：表 17 → 20 张。SMOKE-core-07 的表数量断言与部署方案 §8.2 同步更新。
