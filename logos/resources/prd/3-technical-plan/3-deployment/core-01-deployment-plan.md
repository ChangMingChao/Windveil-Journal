...
| 数据库迁移 | `alembic current` 命中目标 revision；17 张表与 28 个索引齐备 | 第五节 |
...
| 数据隔离 | 新建第二个匿名账号，用其 token 访问第一个账号的愿望，必须 404 | S05 EX-3.1 |
| 加密落地 | 直接查库确认 `wishes.title_enc` 不是可读明文 | `schema.sql` 加密约定 |
| 清理 | 彻底删除 smoke 账号的愿望 → 媒体对象与记录均不存在 | S05.2 Step 25–31 |
