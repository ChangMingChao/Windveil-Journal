...
> 上游：`../../1-product-requirements/core-01-requirements.md`（S01–S07）、
...
frontend/                      React 19 + Vite（纯静态产物）
  ...
  src/components/              愿望卡、状态徽标、半屏、时间线、记忆页
...
- **结构化输出**：用 `response_format={"type": "json_schema"}`（OpenAI 兼容层通用）约束 `WishUnderstanding`，服务端再用 Pydantic 二次校验；校验失败等同 `None`，绝不把半结构化结果写库。
- **待补理解队列**：降级产生的记录写入 `pending_understanding` 表，由 Scheduler 低频重试，对应产品文案里的「稍后再慢慢读它」。
...
|------|-----------|---------|---------|------|
| LLM | 任意 OpenAI 兼容 `/v1/chat/completions` | S01, S02, S04, S06 | `mock-service` | 本地起 OpenAI 兼容 mock，返回固定结构化 JSON；另提供「强制失败」开关以覆盖全部降级验收条件 |
...
| 时间 | 系统时钟 | S03, S04, S06 | `fixed-value` | 时机到达、60 天停滞、发生日期早于种下日期等断言必须可注入时间，服务层统一走 `Clock` 抽象 |
